      ...
      >
        ...
        <el-form-item prop="confirmPassword">
          <el-input
            ...
            clearable
            maxlength="20"
            @input="filterPassword2"
            @compositionstart="confirmComposing = true"
            @compositionend="confirmComposing = false; filterPassword2()"
          />
        </el-form-item>
        <el-form-item>
          <el-button
...
import { User, Lock, Reading, Postcard } from '@element-plus/icons-vue'
...
const confirmComposing = ref(false)
...
const filterPassword2 = () => {
  if (confirmComposing.value) return
  form.confirmPassword = form.confirmPassword.replace(/[^\x21-\x7E]/g, '')
}
...
.login-header h2 { font-size: 21px; color: #1f2d3d; font-weight: 700; }
.login-header p { margin-top: 8px; font-size: 11px; color: #909399; letter-spacing: 3px; }
.login-card :deep(.el-input__wrapper) { border-radius: 10px; transition: box-shadow 0.25s; }
.login-card :deep(.el-input__wrapper:hover) { box-shadow: 0 0 0 1px #a0cfff inset; }
.login-btn {
  width: 100%;
  height: 44px;
  font-size: 16px;
